	...
	for _, tenant := range plan.Tenants {
		for _, topic := range tenant.Topics {
			...
			if topic.ServiceAccountManifestYAML != "" {
				if err := os.WriteFile(filepath.Join(topicDir, "serviceaccount.yaml"), []byte(topic.ServiceAccountManifestYAML), 0o644); err != nil {
					return fmt.Errorf("write serviceaccount.yaml for %s/%s: %w", tenant.Name, topic.Name, err)
				}
			}
