...
- Repeated effective `serviceAccountName` values are now rejected during validation because rendered `ServiceAccount` objects carry topic-specific ownership metadata and therefore need a single owning topic under the current per-topic artifact model.
