...
- Explicit workload identity binding now also emits a matching rendered `ServiceAccount` manifest whenever a topic resolves a non-empty effective `serviceAccountName`, keeping generated bundles self-contained for the common service-account provisioning path while still leaving RBAC out of scope.
- Rendered ServiceAccounts now follow the same narrow inherited metadata model for annotations: operators can set shared and topic-level `serviceAccountAnnotations` values, plus topic `removeServiceAccountAnnotations`, so IAM/workload-identity metadata can land on generated identities without post-render patching.
- ServiceAccount object labels are now first-class too: operators can set shared and topic-level `serviceAccountLabels` values, plus topic `removeServiceAccountLabels`, so rendered identities can carry ownership, GitOps, or policy labels without mutating Deployments, Services, or ConfigMaps.
- Repeated effective `serviceAccountName` values are now allowed as shared workload bindings without duplicate ownership conflicts: Deployments still reference the shared name, the plan emits one top-level shared `ServiceAccount` manifest per repeated compatible name, and `render -outdir` writes the deduplicated artifact under `shared/serviceaccounts/<name>/serviceaccount.yaml` instead of duplicating per-topic files.
- Shared owned `ServiceAccount` output stays intentionally narrow: contributing topics must still resolve identical effective `serviceAccountAnnotations` and `serviceAccountLabels`, while topic-scoped identity labels do not flow onto the shared object because that manifest is plan-scoped rather than topic-scoped.
- Shared owned `ServiceAccount` plan entries now also include contributing tenant/topic refs, so operators can trace which workloads claim a deduplicated shared identity directly from `plan.json` or the REST plan response.
- OPA container image pull behavior is now part of that same narrow Deployment contract: operators can set shared and topic-level `imagePullPolicy` values so rendered workloads can express `Always`, `IfNotPresent`, or `Never` without downstream patching.
- Generated ConfigMaps also need the same narrow inherited metadata model as other rendered objects; shared `controlPlane.configMapAnnotations` plus topic-level `configMapAnnotations` overrides cover reloader, ownership, or GitOps metadata without widening into arbitrary ConfigMap shape changes.
- ConfigMap labels are now first-class too: operators can set shared and topic-level `configMapLabels` for object-scoped discovery, GitOps, or policy metadata without leaking those labels onto Services, Deployments, or pod templates.
...
- Topics can now explicitly remove inherited Service, ConfigMap, Deployment, pod, and ServiceAccount annotation/label keys through narrow `remove*Annotations` and `remove*Labels` lists, so one workload can drop a shared metadata default back to absent state without downstream patches.
- Workload control now also supports shared and topic-level `autoscaling` blocks that render HorizontalPodAutoscaler manifests with CPU and/or memory utilization targets plus optional scale-up/scale-down stabilization windows plus narrow `selectPolicy` and explicit scaling `policies`, so generated OPA Deployments can scale beyond fixed replica counts without downstream patching. Autoscaled workloads must have effective inherited `opaResources.requests.cpu` and/or `opaResources.requests.memory` values for the utilization metrics they configure, and autoscaling behavior now stays intentionally narrow to practical HPA behavior controls instead of broad passthrough.
