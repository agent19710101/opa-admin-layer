...
- Workload control now also supports shared and topic-level `autoscaling` blocks that render HorizontalPodAutoscaler manifests with CPU utilization targets, so generated OPA Deployments can scale beyond fixed replica counts without downstream patching. Autoscaled workloads must also have an effective `opaResources.requests.cpu` value so the CPU-targeted HPA contract is complete.
