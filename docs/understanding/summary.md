...
- Repeated effective `serviceAccountName` values are now allowed as shared/external workload bindings: Deployments still reference the shared name, but the renderer suppresses `ServiceAccount` manifests for repeated names so the plan does not claim conflicting ownership for one Kubernetes object.
- That shared-name contract is still intentionally incomplete, but the next change is now concrete: add a plan-scoped shared `ServiceAccount` ownership model that emits one shared object per repeated effective name when all contributing topics resolve identical effective annotations and labels, and fail early when they disagree.
