...
- Workload control now also supports shared and topic-level `autoscaling` blocks that render HorizontalPodAutoscaler manifests with CPU and/or memory utilization targets plus optional scale-up/scale-down stabilization windows, so generated OPA Deployments can scale beyond fixed replica counts without downstream patching. Autoscaled workloads must have effective inherited `opaResources.requests.cpu` and/or `opaResources.requests.memory` values for the utilization metrics they configure, and autoscaling behavior stays intentionally narrow to stabilization windows instead of broad HPA policy passthrough.
