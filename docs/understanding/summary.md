...
- That shared-name contract is still intentionally incomplete: the next architecture simplification is a plan-scoped shared `ServiceAccount` ownership model that can emit one shared object per effective name once metadata compatibility rules are explicit.
