...
28. Operators should be able to declare shared and topic-level `serviceAccountName` values for rendered OPA Deployments so workload identity and RBAC binding can be selected without downstream patches, including shared names reused across topics as binding-only references when the renderer should not claim ServiceAccount ownership.
