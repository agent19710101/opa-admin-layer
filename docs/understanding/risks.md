...
- Topic labels, Service annotation keys, shared/topic deployment and pod annotation keys, and the shared namespace field are now validated against Kubernetes metadata/name syntax before render, but future metadata expansion (selectors, per-topic namespaces, deployment labels, or deletion semantics for inherited metadata) will need equally explicit guardrails to avoid generating invalid manifests.
