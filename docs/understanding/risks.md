...
- Topic labels, Service annotation keys, shared/topic pod annotation keys, and the new shared namespace field are now validated against Kubernetes metadata/name syntax before render, but future metadata expansion (selectors, per-topic namespaces, deployment annotations) will need equally explicit guardrails to avoid generating invalid manifests.
