...
- whether workload metadata control should later expand beyond shared/topic `deploymentAnnotations` and pod-template `podAnnotations` into deployment labels, pod labels, deletion semantics for inherited metadata, or other admission-controller integration knobs
