...
- whether workload control should later expand beyond shared/topic `replicas`, `deploymentAnnotations`, and pod-template `podAnnotations` into deployment labels, pod labels, deletion semantics for inherited metadata, autoscaling, or other admission-controller integration knobs
