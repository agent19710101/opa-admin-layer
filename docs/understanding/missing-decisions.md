...
- whether workload control should later expand beyond shared/topic `replicas`, `deploymentAnnotations`, pod-template `podAnnotations`, and pod-template `podLabels` into deployment labels, deletion semantics for inherited metadata, autoscaling, or other admission-controller integration knobs
