...
Architecture update (2026-03-26, ConfigMap annotations): generated ConfigMaps now treat annotations as a narrow shared control-plane metadata layer instead of leaving ConfigMap-integrator metadata to downstream patches. The control plane can set shared `configMapAnnotations`, validation reuses the Kubernetes metadata-key contract for annotation keys, and rendering writes the annotations only to `ConfigMap.metadata.annotations` so the slice stays focused on reloader/GitOps-style integration without widening into full object customization.
