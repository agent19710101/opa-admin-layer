...
Architecture update (2026-03-27, OPA autoscaling): workload control now extends beyond fixed replicas into a narrow inherited autoscaling contract. The control plane can set shared `autoscaling` defaults, topics can replace that block entirely, rendering emits a Kubernetes `HorizontalPodAutoscaler` targeting the generated Deployment, and validation keeps the first slice tight by supporting only CPU utilization targets while making `replicas` incompatible with autoscaling-managed workloads. Because that HPA shape is CPU-based, validation now also requires an effective `opaResources.requests.cpu` value for any autoscaled workload after shared/topic inheritance is applied.
