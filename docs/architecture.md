...
Architecture update (2026-03-27, repeated service-account names): repeated effective `serviceAccountName` values are now rejected during validation. Generated ServiceAccounts carry renderer-owned identity labels and propagated topic metadata, so a repeated name would imply conflicting ownership of one Kubernetes object rather than a harmless duplicate artifact. The current architecture therefore assigns each effective rendered ServiceAccount to exactly one owning topic and leaves any future shared-object deduplication model out of scope.
