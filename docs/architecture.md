...
Architecture update (2026-03-27, topic listen-address overrides): listen-address control no longer stops at the shared control-plane default. Topics can now override `controlPlane.defaultListenAddress` with their own `listenAddress`, validation applies the same narrow socket contract at topic scope, and rendering derives each topic's Deployment args, probes, and Service ports from its effective listen address so one workload can move to a different port without forking the rest of the spec.
...
Architecture update (2026-03-26, pod template labels): rendered Deployments now treat pod-template labels as a separate inherited workload-metadata layer instead of forcing every pod-only label through the broader topic-label propagation path. The control plane can set shared `podLabels`, topics can override them, rendering keeps the built-in identity labels immutable so selectors stay stable, and the resulting labels only land on `Deployment.spec.template.metadata.labels` rather than every rendered object.

Architecture update (2026-03-26, deployment labels): rendered Deployments now treat top-level Deployment labels as their own inherited workload-metadata layer instead of forcing deployment-only metadata through broad topic labels or pod-template labels. The control plane can set shared `deploymentLabels`, topics can override them, rendering keeps the built-in identity labels immutable so selectors and ownership metadata stay stable, and the resulting labels land only on `Deployment.metadata.labels` rather than Services, ConfigMaps, or pod templates.

Architecture update (2026-03-26, topic ConfigMap annotations): generated ConfigMaps now follow the same inherited metadata pattern as Services and Deployments instead of stopping at shared-only metadata. The control plane can set shared `configMapAnnotations`, topics can override or extend them key-by-key, validation reuses the Kubernetes metadata-key contract at both scopes, and rendering still writes the annotations only to `ConfigMap.metadata.annotations` so the slice stays focused on ConfigMap-object integration metadata without widening into full object customization.

Architecture update (2026-03-26, service account names): rendered Deployments now treat Kubernetes service-account binding as inherited workload identity metadata alongside replicas, annotations, and pod-template labels. The control plane can set a shared `serviceAccountName`, topics can override it, and validation keeps the slice intentionally narrow by requiring Kubernetes-compatible DNS-subdomain names while leaving service-account creation and RBAC generation out of scope.

Architecture update (2026-03-26, service-account token automount): rendered Deployments now treat `automountServiceAccountToken` as inherited workload identity behavior alongside `serviceAccountName`. The control plane can set a shared boolean, topics can override it explicitly, and rendering keeps the slice narrow by writing only `Deployment.spec.template.spec.automountServiceAccountToken` without widening into arbitrary pod-spec passthrough.

Architecture update (2026-03-27, rendered service accounts): explicit workload identity binding no longer stops at a Deployment field only. When a topic resolves a non-empty effective `serviceAccountName`, the plan now also emits a matching Kubernetes `ServiceAccount` manifest and writes it as a per-topic artifact so the generated bundle stays self-contained for the common identity-binding case. The slice intentionally keeps a per-topic artifact model even when names repeat and still leaves RBAC generation out of scope.

Architecture update (2026-03-27, shared service-account bindings): repeated effective `serviceAccountName` values are now treated as shared/external workload bindings instead of a validation failure. Deployments still inherit and render the shared `serviceAccountName`, but the renderer suppresses `ServiceAccount` manifests for names claimed by more than one topic so the plan does not pretend to own one Kubernetes object with multiple topic identities. The current architecture still leaves true shared-object deduplication, shared metadata merge rules, and RBAC generation out of scope.

Architecture weakness note (2026-03-27, shared service-account ownership): the current shared-name contract solves the duplicate-ownership bug by dropping back to binding-only behavior, but it still cannot represent the common case where the plan should own one shared `ServiceAccount` and let multiple topics reference it. The next simplification should introduce a plan-scoped shared identity layer keyed by effective `serviceAccountName`, emit at most one shared `ServiceAccount` manifest per name, and fail fast when shared topics disagree on shared-object metadata such as annotations or labels.

Architecture update (2026-03-27, shared ServiceAccount ownership): repeated compatible effective `serviceAccountName` values are no longer binding-only. The plan now carries a top-level `sharedServiceAccounts` collection, rendering emits one shared `ServiceAccount` manifest per repeated compatible name, and `render -outdir` writes those owned artifacts under `shared/serviceaccounts/<name>/serviceaccount.yaml` while each topic Deployment continues to reference the same shared name. To keep the shared object contract explicit and deterministic, topic-scoped identity labels do not flow onto shared ServiceAccount objects; only shared ServiceAccount metadata plus plan-scoped built-in identity labels are rendered there.

Architecture update (2026-03-27, shared ServiceAccount traceability): shared owned `ServiceAccount` entries now also record the contributing tenant/topic refs in plan output. This keeps deduplicated shared-object ownership auditable in `plan.json` and REST `/v1/plans` responses without widening the renderer into broader ownership metadata or RBAC generation.

Architecture update (2026-03-27, ServiceAccount annotations): rendered ServiceAccounts now follow the same inherited metadata pattern as Services, ConfigMaps, Deployments, and pod templates for annotations. The control plane can set shared `serviceAccountAnnotations`, topics can override or remove inherited keys through `removeServiceAccountAnnotations`, and rendering keeps the slice intentionally narrow by writing only `ServiceAccount.metadata.annotations` so common IAM/workload-identity integrations work without widening into labels or RBAC generation.

Architecture update (2026-03-27, ServiceAccount labels): generated ServiceAccounts now treat object labels as their own inherited metadata layer instead of stopping at annotations only. The control plane can set shared `serviceAccountLabels`, topics can override or remove inherited keys through `removeServiceAccountLabels`, rendering carries the existing built-in identity labels and propagated topic labels onto `ServiceAccount.metadata.labels`, and renderer-owned identity labels remain immutable so cross-object discovery stays stable even when custom labels overlap.

Architecture update (2026-03-27, image pull policy): rendered Deployments now treat `imagePullPolicy` as inherited workload runtime behavior alongside `opaImage`, `serviceAccountName`, and `automountServiceAccountToken`. The control plane can set a shared Kubernetes pull policy default, topics can override it, validation keeps the slice intentionally narrow to `Always`, `IfNotPresent`, or `Never`, and rendering writes only `Deployment.spec.template.spec.containers[].imagePullPolicy` without widening into arbitrary container customization.
...
Architecture update (2026-03-27, inherited metadata removals): inherited object metadata is no longer add-or-override only. Topics can now remove shared Service, ConfigMap, Deployment, and pod annotation/label keys through explicit `remove*Annotations` and `remove*Labels` lists, the renderer applies those removals after shared/topic merge so one workload can drop a shared default back to absent state, and renderer-owned identity labels remain outside that contract so selectors and object identity stay stable.
Architecture update (2026-03-27, OPA autoscaling): workload control now extends beyond fixed replicas into a narrow inherited autoscaling contract. The control plane can set shared `autoscaling` defaults, topics can replace that block entirely, rendering emits a Kubernetes `HorizontalPodAutoscaler` targeting the generated Deployment, and validation keeps the slice tight by allowing only CPU and memory utilization targets while making `replicas` incompatible with autoscaling-managed workloads. Because those HPA resource metrics depend on Kubernetes utilization baselines, validation now requires effective inherited `opaResources.requests.cpu` and/or `opaResources.requests.memory` values for the autoscaling metrics a workload configures.

Architecture update (2026-03-27, HPA stabilization windows): the autoscaling contract now also carries an intentionally narrow `behavior` sub-block for scale-up and scale-down stabilization windows. Operators can express `stabilizationWindowSeconds` without opening broader HPA policy passthrough, rendering keeps the behavior block deterministic when configured, and validation treats empty behavior objects or negative/oversized window values as contract violations instead of leaking malformed policy into generated HPAs.

Architecture update (2026-03-27, HPA scaling policies): the autoscaling behavior contract now also allows narrow `selectPolicy` and explicit `policies[]` entries on `scaleUp` and `scaleDown`. This keeps behavior tuning operator-focused — step-size and policy selection for Pods/Percent scaling — while still avoiding full HPA passthrough, and validation rejects unsupported select-policy values or malformed scaling-policy entries before they reach rendered manifests.
