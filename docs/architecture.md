...
Architecture note (2026-03-27, repeated service-account names): the current per-topic artifact model becomes ambiguous when multiple topics resolve the same `serviceAccountName` but would render different annotations or labels. That is now an explicit architecture weakness rather than an incidental renderer detail: a future slice should either reject conflicting repeated names during validation or introduce a shared-object ownership model with deterministic merge rules. Until then, repeated names remain tolerated but intentionally undocumented as a safe customization path.
