...
Architecture update (2026-03-23, service annotation gap): configurable Service type exposed the next networking integration weakness: generated Services still cannot carry controller-specific annotations, so operators must patch the exact manifest the renderer now owns. The next slice should add a shared control-plane `serviceAnnotations` map, validate it with the same Kubernetes metadata guardrails used elsewhere, and keep the scope intentionally narrow to Service metadata only.
